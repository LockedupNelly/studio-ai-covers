...
const CREATIVE_VARIANT_DESCRIPTIONS: Record<number, string> = {
  ...
  5: "TYPOGRAPHY STYLE: BOLD SANS-SERIF with CRACKED/SHATTERED texture AND TRANSPARENT GAPS. CRITICAL - MUST BE SANS-SERIF: Heavy bold geometric sans-serif letterforms (like Impact or Bebas Neue), NO serifs, NO neon, NO glow effects. LETTERFORMS: Thick, bold, uppercase sans-serif letters with strong presence. CRITICAL CRACKED EFFECT WITH GAPS: The letters MUST have TWO types of damage: (1) Fine cracks and fractures running through the surface texture, AND (2) At least ONE or TWO LARGE BREAKS/GAPS that completely split through the letters creating TRANSPARENT HOLES where the background image shows through. These gaps are like chunks missing from the letters - actual transparent openings, not just surface cracks. The large breaks should cut diagonally or vertically through parts of the text. TEXTURE: Grungy, distressed, cracked surface with both fine cracks AND large transparent gaps/holes. COLOR: White or off-white text with visible damage, on dark background. MOOD: Broken, shattered, dramatic destruction aesthetic. NO NEON, NO GLOW.",
